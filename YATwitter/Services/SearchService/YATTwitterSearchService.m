...
- (void)getTweetsForUsername:(nonnull NSString*)username completion:(nonnull YATTwitterServiceTweetsCompletion)completion {
    ...
    NSString* format = [NSString stringWithFormat:@"https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=%@&count=100", [username stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]];
...
- (void)getTweetsByWord:(nonnull NSString*)word completion:(nonnull YATTwitterServiceTweetsCompletion)completion {
    NSString* authorization = [@"Bearer " stringByAppendingString:self.state.token.value];
    NSString* format = [NSString stringWithFormat:@"https://api.twitter.com/1.1/search/tweets.json?q=%@&count=100", [word stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]];
    NSURL* url = [NSURL URLWithString:format];
    NSMutableURLRequest* request = [NSMutableURLRequest requestWithURL:url];
    request.HTTPMethod = @"GET";
    [request addValue:authorization forHTTPHeaderField:@"Authorization"];
